...
{
    template <typename FormatCtx> auto format(const ae::xlsx::cell_addr_t& addr, FormatCtx& ctx) const { return fmt::format_to(ctx.out(), "{}{}", addr.col, addr.row); }
};

template <> struct fmt::formatter<ae::xlsx::cell_match_t> : fmt::formatter<ae::fmt_helper::default_formatter>
{
    template <typename FormatCtx> auto format(const ae::xlsx::cell_match_t& match, FormatCtx& ctx) const { return fmt::format_to(ctx.out(), "{}{}:{}", match.row, match.col, match.matches); }
};
