...
template <> struct fmt::formatter<ae::sequences::pos1_t> : fmt::formatter<ae::fmt_helper::default_formatter> {
    template <typename FormatCtx> auto format(const ae::sequences::pos1_t& pos1, FormatCtx& ctx) const { return fmt::format_to(ctx.out(), "{}", pos1.get()); }
};

template <> struct fmt::formatter<ae::sequences::pos0_t> : fmt::formatter<ae::fmt_helper::default_formatter>
{
    template <typename FormatCtx> auto format(const ae::sequences::pos0_t& pos0, FormatCtx& ctx) const { return fmt::format_to(ctx.out(), "{}", ae::sequences::pos1_t{pos0}); }
};
