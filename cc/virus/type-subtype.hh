...
{
    template <typename FormatContext> auto format(const ae::virus::type_subtype_t& ts, FormatContext& ctx) const { return fmt::format_to(ctx.out(), "{}", static_cast<std::string_view>(ts)); }
};
