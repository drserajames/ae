...
| 8 | **Tree pages render ~5% wider than AD — a regression, not a gap** | `cc/tal/draw-tree.cc:796-797`, `py/ae/tal/settings_v3.py:889` | The aa-label reserve is **added to the page width** instead of taken out of the tree: `aa_band = 0.05 * width_base` (:796) then `const double width = width_base + aa_band;` (:797). AD draws a page of exactly `height x width_to_height_ratio` (`AD/sources/acmacs-tal/cc/draw.cc`), so every ae tree page comes out at exactly **1.05x** AD's for the same input. Measured by `eu-70` on the Feb-2026 round, same `.tal`+`.tjz` through both engines (pt, ae vs AD): bvic **663.2 vs 631.6**, h1 **834.0 vs 794.3**, h3 **681.0 vs 648.6** -- ratio **1.0500 on all three, to four figures**, which is what points at a constant rather than data-dependent drift. **This is a regression**: §3 above records exact page widths as matched vs AD on 2026-06-17 at precisely AD's numbers (631.6 / 648.6 / 794.3), so the band was added afterwards. Secondary contributor: `width_to_height_ratio` is rounded to 4 dp (:889), worth up to ~0.05pt of page width against AD. **Cost:** padded to a common canvas ~18-20% of the page differs, so **no whole-page pixel-parity claim against AD is possible on trees at all**, which is what makes the `p2-fidelity` scoreboard meaningless for tree renders and forced a recent verification back onto structural checks (shown-leaf sets, `V` ranges, section anchors). Pre-existing and unrelated to PR #70 -- identical before and after. **Fix in progress, uncommitted and unverified**, in `~/AC/eu/ae-page-size` (branch `page-size-parity`, an `eu-70` subagent): it makes the band an inset of the drawable area and raises the rounding to 6 dp. That branch is **9 behind / 0 ahead** of `main`, and PR #70 rewrote both files around the anchor lines, so it needs a rebase + rebuild before any ae-vs-AD number taken there means anything. Still open on `main`. Background: `~/AC/eu/AE-PAGE-SIZE-HANDOFF.md`. |
