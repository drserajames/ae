...
## Open defects and environment issues (#1–#7 found 2026-09-10; #2, #4–#7 fixed unmerged, #3 partially, as of the third pass. **#8–#9 added 2026-09-15 — seqdb defects, both open and unfixed**)
...
| 9 | ~~**`Seqdb::add` treats a hash match as proof of sequence equality**~~ — data loss, **FIXED, unmerged** (`seqdb-collision-fix`) | `cc/sequences/seqdb.cc:72-120`, `cc/sequences/seqdb-selected.hh` | **Was:** the incoming sequence is looked up by `hash_nuc`; `keep_sequence` is then set from **the hash hit plus a name comparison only**, and **no nucleotide comparison happens anywhere in `add()`**. The only rescue was the nucleotide-*length* guard, written as an **`else if`**, so it fired *only* when the lengths differed. On a same-length collision the incoming sequence was stored as a slave with `nuc` cleared (`dont_keep_sequence()`, `seqdb.hh:240`) and `SeqdbSeqRef::set_master()` (`seqdb.hh:317-321`) later resolved that slave **to a different virus's sequence** — silently, because the `>> [seqdb …] hash conflict …` diagnostic sits *inside* the length-differs branch. **Fixed** by comparing the actual nucleotides on a hash hit and splitting the four cases: (a) no hit, or hit under the same name → keep and register in `hash_index_`; (b) hit, different name, nucleotides **equal** → genuine duplicate, slave, unchanged — this is the point of the design; (c) hit, unequal, **lengths differ** → the existing truncated-sequence policy, unchanged; (d) hit, unequal, **same length** → the defect: **both sequences kept**, the incoming one becomes its own master and is deliberately **not** registered in `hash_index_`, so the hash's canonical master and every slave already resolving through it are untouched, and it reports loudly. **A second site carried the same wrong assumption one layer up:** `SeqdbSelected::remove_hash_duplicates()` deduped on `hash` alone, so it would have discarded the second sequence again, in the export selection, immediately after `add()` started keeping it — fixing `add()` alone changes nothing downstream. It now also requires the nucleotides to match before treating two **masters** as duplicates; a slave still dedups against its master, because post-fix a slave is only ever created for a genuine duplicate. **Verified** by replaying all 490 GISAID files under `ac/sequences` through the production `translate`/`align` path into a fresh empty seqdb, with the pre-fix and post-fix builds in the same session: **1,989,923 records read, 1,977,023 aligned+hashed, 310,524 distinct hashes, 5 same-length collisions** (read-count and distinct-hash count reproduce the original report exactly). **Pre-fix vs post-fix, both halves replayed in the same session against the same input:** the two runs read identical totals (1,989,923 / 1,977,023 / 310,524) and store an identical number of entries, so only the master/slave decision differs. Hashes carrying **two or more masters**: **0 → 5** (2 in A(H1N1) at 1647 nt, 3 in A(H3N2) at 1650 nt). Across those 5 hashes, refs stored with `stored_nuc=0` fall **7 → 2**, i.e. **5 sequences recovered**; the 2 that remain slaves are genuine duplicates and *should* stay slaves, so the fix does not over-correct. Guarded by `cc/test/test-seqdb-collision.cc`, which passes 6/6 here and fails exactly the two collision checks against an unmodified 41b03d6 build. Genuine duplicates are unaffected — they remain slaves. **The hash is NOT widened:** `cc/tree/tree.cc:484` slices it out of the last 8 characters of `seq_id`, so widening renames every newick tip and breaks weekly-tree continuity, and it only makes collisions rarer, not impossible. Considered and rejected. **Still outstanding — this is a code fix only:** the records already damaged in the stored seqdb are NOT repaired. A rebuild was ruled out by Sarah on 2026-09-15, so those strains stay damaged and the trees keep reflecting them until that decision changes. |
| 10 | ~~**`.exclude_name` runs before `.replace_with_master`**~~ — excluded strains re-enter the tree, **FIXED, unmerged** (`seqdb-collision-fix`) | `proj/weekly-tree/export` (all four chains) | **Was:** `.exclude_name([…])` ran **before** `.replace_with_master()`, and `replace_with_master` resolves by hash **across the whole seqdb, ignoring every prior filter** — so an excluded strain that happens to be the hash-master of an included one comes straight back in. Not collision-dependent: it happens on every export whenever an excluded strain is the hash-master of an included one, so it is independent of #9. **All four chains (bvic, byam, h1, h3) shared the ordering**, so all four are fixed. **Fixed** by RE-APPLYING the exclusion after the substitution rather than moving it: the original call has to keep running before `remove_hash_duplicates()`, or an excluded strain could win the dedup against an included one and the included strain would then be lost when the trailing exclusion removed it — trading this defect for a new one. **Verified** on the real H3 selection against the current seqdb: **9 → 0** excluded non-human sequences in the human H3 tree input, with the total selection falling by exactly 9 (77,964 → 77,955), so nothing else was dropped. **Review note:** on bvic only, this lets the exclusion override the `include_name(outgroup)` that follows it. That bites only if a name appears in both the outgroup and `bvic-exclude.txt`, which would be contradictory curation, but it is a behaviour change and that list is a runtime input not visible from the repo. |
