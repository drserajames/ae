...
| 8 | **`Seqdb::add` treats a hash match as proof of sequence equality** — data loss, **open, not fixed** | `cc/sequences/seqdb.cc:72-108` | The incoming sequence is looked up by `hash_nuc`; `keep_sequence` is then set from **the hash hit plus a name comparison only** — a hash hit under a different name already means "discard". **No nucleotide comparison happens anywhere in `add()`.** The only thing that rescues a collision is the length guard, and it is an **`else if`**, so it fires *only* when the two nucleotide lengths differ. **On a same-length collision the incoming sequence is stored as a slave with `nuc` cleared** (`dont_keep_sequence()`, `seqdb.hh:240`), and `SeqdbSeqRef::set_master()` (`seqdb.hh:317-321`) later resolves that slave **to a different virus's sequence**. Worse, the `>> [seqdb …] hash conflict …` diagnostic sits *inside* the length-differs branch, so **same-length collisions produce no message at all** — the failure is completely silent. **Measured** (all 490 GISAID files under `ac/sequences`, replayed through the production `translate`/`align` path, hashing before `add` could absorb): 1,989,923 records read, 1,794,103 aligned+hashed, 310,524 distinct hashes, **7 collisions, 5 of them same-length and therefore silent**. Nucleotide-length homogeneity is ~85% modal per subtype, so the guard structurally misses most collisions (5/7 observed). **Live damage in the current seqdb: 4 strains stored with `stored_nuc=0`, sequence discarded; a 5th partially affected.** It reaches the trees: for one collided pair the human H3 tree input **loses the human strain and gains a non-human one**. **Fix direction (analysis only — nothing implemented):** compare actual nucleotides on a hash hit and split the four cases explicitly — (a) no hash hit, or hash hit under the *same* name → keep, register in `hash_index_`; (b) hash hit, different name, **nucleotides equal** → genuine duplicate, slave, current behaviour is correct and is the point of the design; (c) hash hit, different name, nucleotides differ, **lengths differ** → the existing truncated-sequence policy, unchanged; (d) hash hit, different name, nucleotides differ, **same length** → the defect: keep both, the incoming sequence becomes its own master (do *not* register it in `hash_index_`, so the hash's canonical master is untouched and no existing slave is disturbed), and report loudly. Verified the comparison is expressible as written: both sides are `sequence_nuc_t` (`sequence.hh:43`) and `named_string_t` inherits `operator==` (`cc/utils/named-type.hh:75`). **Do not widen the hash to xxhash64** — `cc/tree/tree.cc:484` slices the hash out of the last 8 characters of `seq_id`, so widening renames every newick tip and breaks weekly-tree continuity, and it only makes collisions rarer, not impossible. Considered and rejected. |
| 9 | **`.exclude_name` runs before `.replace_with_master`** — excluded strains re-enter the tree, **open, not fixed** | `proj/weekly-tree/export` (h1 lines 141/143, h3 lines 164/166) | `.exclude_name([…])` is called **before** `.replace_with_master()`, and `replace_with_master` resolves by hash **across the whole seqdb, ignoring every prior filter** — so an excluded strain that happens to be the hash-master of an included one comes straight back in. **Measured: 9 non-human sequences are present in the human H3 tree input.** Not collision-dependent — it happens on every export whenever an excluded strain is the hash-master of an included one, so it is independent of #8 and fixable on its own. Swapping the two calls is the obvious fix; **check `bvic` (86/91) and `byam` (114/116) for the same ordering before settling the shape of it.** |
