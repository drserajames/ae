...
| 16 | **`Tree::remove` leaves one-child internal nodes; cmaple 1.0.0 segfaults on them** — **open, claimed on `weekly-tree-prune`** | `cc/tree/tree.cc` (`Tree::remove`), `proj/weekly-tree/make-cmaple` (`prune_source_tree`) | **Mechanism, read from the code:** a post-order walk over inodes; an inode left with exactly one child is pushed on `single_child_inodes`; at its parent a single `find_if` replaces **only the first** pending child with that child's own child (adding the edge lengths). When two or more sibling groups under one parent are each reduced to one member, only the first is collapsed and the rest stay as `(X:e)` nodes for good — the code's own comment anticipates "multiple single_child_inodes … for the same parent" but does not handle it. A single-child root is not handled either. Leaf set and root-to-tip distances are otherwise correct. **Synthetic reproduction:** `(((A:1,B:1):1,(C:1,D:1):1):1,E:1);` removing B and D gives `((A:2,(C:1):1):1,E:1);` instead of `((A:2,C:2):1,E:1);`; three such sibling groups leave 2. Random trees (20–400 tips, 1–20 % removed): 107 of 300 kept ≥1 one-child node; 0 had a wrong leaf set or changed root-to-tip distance. **Consequence, measured 2026-09-17:** `make-cmaple` prunes last week's tree to this week's alignment with `tree.remove`; one subtype's pruned tree had 6 one-child nodes and cmaple 1.0.0 segfaulted within 3 s at "Reading a tree". The same tree with only those 6 nodes collapsed loaded and ran; the other two subtypes (0 such nodes) ran normally. A large exclude-list change had forced ~230 tips to be pruned. Worked around that week by editing the exclude lists; **not fixed**. |
| 17 | **`proj/weekly-tree/make-cmaple` reports success after a failed cmaple run** — **open, claimed on `weekly-tree-prune`** | `proj/weekly-tree/make-cmaple`, `proj/weekly-tree/init` | `submit()` catches each run's exception, logs it and counts `failures`, but never raises. `cmaple()` then writes `cmaple_tree.txt` pointing at a `000.treefile` that does not exist, `main()` sends the **"completed"** mail, and the script exits 0 — so `submit`'s `&&` chain carries on into `rotate.R`, which fails with a file-not-found that hides the real cause. A failed week folder also poisons the next week: `init`'s `find_previous_dir` takes the newest `YYYY-MMDD` folder and links `previous.newick` to the missing file, unless the folder is renamed out of that pattern. Observed 2026-09-17. |
