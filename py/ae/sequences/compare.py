"""
ae.sequences.compare — compare amino-acid / nucleotide sequences of groups of strains.

The comparison itself (per-position counters, the positions worth reporting, the JSON the
viewer consumes) and the self-contained HTML viewer used to live inside
`bin/seqdb-compare-sequences`, reachable only from that command line. They are here so
other code can use them too — `ae.adjust.Adjust.compare_sequences` / `Slot.compare_sequences`
compare two groups of *chart* points, the port of AD's `chart_draw.compare_sequences`.

A group is a name plus a list of `(seq_id, sequence)` pairs, where *sequence* is anything
with `len()`, 1-based `[]` and `str()` — `ae_backend.SequenceAA` / `SequenceNuc`, as
returned by `antigen.sequence_aa()` and by seqdb, or a plain 1-based-indexable wrapper.
"""

import sys, re, collections, json, subprocess
...
class Group:
    """One named group of sequences.

    *seqs* is a list of `(seq_id, sequence)` pairs (or `Group.Seq` objects). Sequences are
    indexed **1-based**, the way `ae_backend.SequenceAA` is, so `counters[pos0]` holds the
    amino acids seen at position `pos0 + 1` — which is the position the JSON, and the
    viewer, report.
    """
    ...
    def __init__(self, name: str, seqs: list):
        self.name = name
        self.seqs = [seq if isinstance(seq, self.Seq) else self.Seq(*seq) for seq in seqs]
        self.longest = max((len(seq.seq) for seq in self.seqs), default=0)
        self.counters = [collections.Counter(seq.seq[pos] for seq in self.seqs if len(seq.seq) >= pos) for pos in range(1, self.longest + 1)]

    @classmethod
    def from_seqdb(cls, name: str, seqs: list, nuc: bool = False):
        "A group of seqdb entries, which carry `seq_id()` and `aa`/`nuc`."
        return cls(name=name, seqs=[(seq.seq_id(), getattr(seq, "nuc" if nuc else "aa")) for seq in seqs])
    ...
    def make_summary(self, positions: list, most_frequent: dict, threshold: float):
        num_rows = max((len(cnt) for cnt in self.counters), default=0)
        rows = []
        ...
        if not rows:
            rows.append([self.name] + ["" for pos in positions])
        return rows
...
class Comparison:
    """A set of `Group`s compared position by position.

    `positions_to_report` are the positions where more than one amino acid (nucleotide)
    occurs across all the groups together — the only positions the summary and the viewer
    show. Port of AD's `acmacs::seqdb::subsets_to_compare_t` (`seqdb-3/cc/compare.hh`),
    including its "subset empty" warning.
    """
    ...
    def positions_to_report(self):
        "0-based indices into the groups' counters; position `n` is reported as `n + 1`."
        for group in self.groups:
            if not group.seqs:
                print(f">> subset empty: {group.name}", file=sys.stderr)
        merged_counters = {pos: collections.Counter() for pos in range(max((gr.longest for gr in self.groups), default=0))}
        for group in self.groups:
    ...
    def format_summary(self, threshold: float = None):
        ...
        most_frequent = self.groups[0].most_frequent(positions=positions) if self.groups else {}
    ...
    def write_html(self, output: Path, open: bool = False) -> Path:
        """Write the self-contained HTML viewer to *output* and return that path,
        optionally opening it."""
        output = Path(output)
        with output.open("w") as out:
            out.write(make_html(data_json=self.format_json(), var_name=html_var_name(output)))
        if open:
            open_html(output)
        return output
...
def html_var_name(output: Path) -> str:
    """The JavaScript variable the generated page puts its data in, derived from the file
    name as AD derives it (`seqdb-3/cc/compare.hh`) — every character that cannot appear
    in an identifier becomes `_`. AD replaces `/`, `-` and `.`; do the lot, because the
    adjust stage's file names (`<stem>.compare-seq.html`) contain a dot in the stem and a
    half-replaced name is not a valid identifier."""
    return "compare_sequences_" + re.sub(r"\W", "_", Path(output).stem)

# ----------------------------------------------------------------------

def open_html(path: Path):
    "Show the generated page (macOS `open`); never fatal."
    try:
        subprocess.call(["/usr/bin/open", str(path)])
    except OSError as err:
        print(f">> cannot open {path}: {err}", file=sys.stderr)

# ----------------------------------------------------------------------

def compare_sequences(groups: dict, output: Path = None, open: bool = False) -> Comparison:
    """Compare *groups* — `{name: [(seq_id, sequence), ...]}` — and, if *output* is given,
    write the HTML viewer there. Returns the `Comparison`."""
    comparison = Comparison([Group(name=name, seqs=seqs) for name, seqs in groups.items()])
    if output is not None:
        comparison.write_html(output, open=open)
    return comparison

# ----------------------------------------------------------------------

class Groups(Comparison):
    "Groups selected from seqdb by name, as `bin/seqdb-compare-sequences` does."

    def __init__(self, sources: dict, nuc: bool):
        self.subtype = sources.get("subtype")
        self.lineage = sources.get("lineage")
        self.groups = [Group.from_seqdb(name=group, seqs=[s2 for s1 in (self.name_to_seq(name, sources) for name in names) for s2 in s1], nuc=nuc) for group, names in sources["groups"].items()]
