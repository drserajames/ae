"""Chain fold logic (ported from acmacs_py.chain202105.chains).

IndividualTableMapChain, IncrementalChain and IncrementalMergeMaker. The fold is
preserved exactly; only chart accessors and the merge call were swapped to
ae_backend.chart_v3 (see maps.py header for the accessor table).
"""
...
class IncrementalChain (ChainBase):
    ...
    def first_map(self, runner, chain_setup):
        chart  = ae_backend.chart_v3.Chart(self.tables[0])
        ...
        map_path = output_dir.joinpath(f"""{self.output_prefix(0)}{maps.chart_date(chart)}{self.tables[0].suffix}""")
    ...
    def choose_between_incremental_scratch(self, incremental_map_output :Path, scratch_map_output :Path, log :Log):
        if incremental_map_output and scratch_map_output:
            incremental_chart = ae_backend.chart_v3.Chart(incremental_map_output)
            incremental_stress = incremental_chart.projection(0).stress()
            scratch_chart = ae_backend.chart_v3.Chart(scratch_map_output)
...
class IncrementalMergeMaker:
    ...
    def make(self, previous_merge :Path, new_table :Path, output_dir :Path, output_prefix :str):
        ...
        previous_chart = ae_backend.chart_v3.Chart(previous_merge)
        chart_to_add = ae_backend.chart_v3.Chart(self.chain_setup.individual_table_preprocess(new_table, output_directory=output_dir))
        if previous_chart.titers().number_of_layers() < 2:
            merge_date = f"""{maps.chart_date(previous_chart)}-{maps.chart_date(chart_to_add)}"""
        else:
            merge_date = f"""{maps.chart_date(previous_chart).split("-")[0]}-{maps.chart_date(chart_to_add)}"""
        self.output_path = output_dir.joinpath(f"{output_prefix}{merge_date}.merge{previous_merge.suffix}")
        if not self.output_path.exists():
            # AD: acmacs.merge(prev, add, type="incremental", ...); ae swaps type->merge_type
            # and returns (Chart, merge_data). merge_data.common() is a report string (logged).
            merge, merge_data = ae_backend.chart_v3.merge(previous_chart, chart_to_add, match="strict", merge_type="incremental", combine_cheating_assays=self.chain_setup.combine_cheating_assays())
            self.log.info(after_newline=merge_data.common())
            merge.write(self.output_path)
            self.column_bases = maps.extract_column_bases(merge)
        else:
            self.column_bases = maps.extract_column_bases(ae_backend.chart_v3.Chart(self.output_path))
        self.log.separator()
        return self
