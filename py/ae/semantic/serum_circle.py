...
def style(chart: ae_backend.chart_v3.Chart, style_name: str, priority: int = 100, sera: list[int] | None = None, fold: float = 2.0, theoretical: bool = False, fallback: bool = True, mark_serum_outline: bool = False, circle_style: dict = {"outline": {"egg": "red", "cell": "blue", "reassortant": "orange"}, "fill": {"egg": "transparent", "cell": "transparent", "reassortant": "transparent"}, "outline_width": 1.0, "dash": 0}) -> set[str]:
    ...
    mark_serum_outline: True - recolour each circled serum's SQUARE outline to that serum's
      passage colour (the same colour used for its circle) and raise it, reproducing AD's
      `mark_serum:{"outline":"passage","order":"raise"}` (mapi.json spc serum-circle block /
      mod-serum.cc). Off by default so other callers keep the serum's existing outline.
    circle_style: {
    ...
    for serum_no in sera:
        if serum_no >= 0 and serum_no < num_sera:
            ...
            # AD mark_serum: recolour THIS circled serum's square outline to its passage colour
            # (the same colour as its circle outline) and raise it — spc.tal
            # `mark_serum:{"outline":"passage","order":"raise"}`. Emitted per-serum here (alongside
            # the circle) rather than as a set-wide modifier so each square matches its own circle.
            if mark_serum_outline:
                mark_outline = this_circle_style.get("outline")
                if isinstance(mark_outline, str):
                    style.add_modifier(selector={"!i": serum_no}, only="sera", outline=mark_outline, raise_=True)
        else:
